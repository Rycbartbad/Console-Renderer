...
// ── 5×5 TTF font (loaded from embedded data at startup) ───────
#include "04B-03-1.ttf.h"
static const uint8_t* g_font5x5 = nullptr;
...
Layer2D::draw_text(float nx, float ny, const std::string& text, Vec3 fg, Vec3 bg, float scale) {
    ...
    if (ch <= 5) { fh = 5; font = g_font5x5; } else { fh = 8; font = s_font8x8; }
...
Renderer::Renderer() {
    Screen::init();
#ifdef _WIN32
    // Load 5×5 font from embedded TTF data (no file I/O)
    DWORD nFonts = 0;
    HANDLE hFont = AddFontMemResourceEx((void*)s_ttf_5x5, sizeof(s_ttf_5x5), nullptr, &nFonts);
    if (hFont && nFonts > 0) {
        ...
        if (hf) {
            ...
            static std::vector<uint8_t> fd(95 * 5, 0);
            for (int c = 32; c < 127; c++) {
                RECT rc = {0, 0, 8, 8}; FillRect(hdc, &rc, (HBRUSH)GetStockObject(BLACK_BRUSH));
                wchar_t wc = (wchar_t)c; DrawTextW(hdc, &wc, 1, &rc, DT_CENTER | DT_VCENTER | DT_NOCLIP);
                GdiFlush();
                uint32_t* px = (uint32_t*)bits;
                for (int row = 0; row < 5; row++) {
                    uint8_t byte = 0;
                    for (int col = 0; col < 5; col++)
                        if ((px[row * 8 + col] & 0xFF) > 128) byte |= (0x80 >> col);
                    fd[(c - 32) * 5 + row] = byte;
                }
            }
