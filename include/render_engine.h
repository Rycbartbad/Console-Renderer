            ...
            for (int ox2 = 0; ox2 < nc; ox2++) {
                ...
                for (int sy = 0; sy < SS; sy++)
                    for (int sx = 0; sx < SS; sx++) {
                        float px = f0x + (f1x - f0x) * (sx + .5f) / SS, py = f0y + (f1y - f0y) * (sy + .5f) / SS;
                        int fi = (int)px, fj = (int)py;
                        if (fi < 0)
                            fi = 0;
                        if (fj < 0)
                            fj = 0;
                        if (fi < 8 && fj < 8 && (s_font8x8[(c - 32) * 8 + fj] & (0x80 >> fi)))
                            hits++;
                    }
                if (!hits)
                    continue;
                float cov = hits * in;
                Vec3 col((int)(fg.x * cov + bg.x * (1 - cov)),
                         (int)(fg.y * cov + bg.y * (1 - cov)),
                         (int)(fg.z * cov + bg.z * (1 - cov)));
